...
Installieren Sie das Modul über den Cognis Module Marketplace oder legen Sie das vollständige Repository im konfigurierten Verzeichnis für externe Module ab. Cognis lädt `bootstrap.js`, das Admin-Bereich und API-Routen über `ctx` registriert. Das Modul benötigt die Host-Capability `db:executor`. Das modul-eigene Ereignisschema wird beim Start sichergestellt; Fehler werden mit sicheren Komponenten- und Operationsmetadaten protokolliert.
