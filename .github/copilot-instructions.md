...
After the final file change, run `npm run manifest:hashes` to regenerate every SHA-256 digest in `manifest.files`. Do not include `manifest.json` in its own digest list. Verify all declared digests before committing. Changelog files under `docs/changelog/` are host-discovered release metadata and must never be added to `manifest.files` or assigned manifest SHA-256 digests. Keep repository, homepage, and support metadata pointed at this project. Do not add generated secrets, credentials, personal data, or unnecessary dependencies.
